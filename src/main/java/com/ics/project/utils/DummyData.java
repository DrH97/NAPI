...
public class DummyData implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        movies.add(new Movie("Jumanji", "A jungle in the wilderness", Utils.getRandomCategories(categories, 1), MovieType.ORIGINAL, savedUsers.get(rand.nextInt(savedUsers.size()))));
        movies.add(new Movie("Joker", "Why so serious???", Utils.getRandomCategories(categories, 2), MovieType.ORIGINAL, savedUsers.get(rand.nextInt(savedUsers.size()))));
        ...
        Utils.print(savedCategories.get(0).toString(), savedCategories.get(1).toString(), savedCategories.get(2).toString()
        );

        Utils.print(savedMovies.get(1).toString(), savedMovies.get(2).toString());
    }
}
